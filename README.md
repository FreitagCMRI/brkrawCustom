This repository is an adaptation designed to suit specific needs encountered while working with data acquired via Bruker. All credits are listed below.
Key Changes:
Version 0.4.0.dev2+freitag
    GUI Enhancements: Added a parameter search feature.

    Parameter Access: Made reconstruction parameters (reco pars) accessible, although not pretty.

    Diffusion Fixes: Fixed diffusion gradient orientation regarding the image relation(specifically for setups using dtiStandard).

        Updated .get_bdata_img(scan_id, reco_id, ...)

        Updated .save_bdata_img(self, scan_id, reco_id, ...)

Installation

To install, run the following command: pip install git+https://github.com/FreitagCMRI/brkrawCustom

TODO:
- Reco Corrections regarding iso center offset.
- ...
---
...
to enable use of the archived data directly.The module is comprised of four components, including graphical user interface (GUI), command-line tools,
